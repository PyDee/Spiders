...
def kuaishou(rid):
    headers = {
        ...
        'cookie': 'did=web_2a570a791d027ad80fa7fc88874a4d65'
    }
    with requests.Session() as s:
        res = s.get('https://m.gifshow.com/fw/live/{}'.format(rid), headers=headers)
        # res = s.get('https://live.kuaishou.com/u/{}'.format(rid), headers=headers)
        livestream = re.search(r'liveStream":(.*)"},', res.text)
        if livestream:
