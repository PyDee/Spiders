            ...
            if 'data' in ret_dict.keys():
                ...
                if total <= 50:
                    for focus_item in focus_list:
                        ...
                        # yield 请求focus内容
                        yield scrapy.Request(url=self.follow_info_url.format(focus.get('focus_id')),
                                             meta=response.meta,
                    ...
                    # yield 直接放第二页的url
                    yield scrapy.Request(url=self.second_page_url.format(user_id),
                                         meta=response.meta,
        ...
        if not status_code:
            if 'data' in ret_dict.keys():
                ...
                for element in item:
                    ...
                    # yield 请求focus内容
                    yield scrapy.Request(url=self.follow_info_url.format(element.get('focus_id')),
                                         meta=response.meta,
                                         callback=self.get_follow_info)
